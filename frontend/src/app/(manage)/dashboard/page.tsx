  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Cập nhật Tỷ lệ Giới tính (Thêm giới tính khác)
        setGenderData({
          labels: ['Nam', 'Nữ', 'Khác'],
          datasets: [{ 
            data: [popRes.data.male, popRes.data.female, popRes.data.other], 
            backgroundColor: ['#3B82F6', '#EC4899', '#94A3B8'] 
          }]
        });

        // Cập nhật Phân bổ theo Nhóm tuổi (Thêm Mới sinh 0-1 và Mầm non 1-5)
        setAgeGroupData({
          labels: ['Mới sinh (0-1)', 'Mầm non (1-5)', 'Cấp 1', 'Cấp 2', 'Cấp 3', 'Lao động', 'Nghỉ hưu'],
          datasets: [{ 
            label: 'Số lượng', 
            data: Object.values(ageRes.data), 
